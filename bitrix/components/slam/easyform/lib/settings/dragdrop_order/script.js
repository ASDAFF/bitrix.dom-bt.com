...
DragNDropOrderParameterControl.prototype =
{
	...
	initDragDrop: function()
	{
		if (BX.isNodeInDom(this.params.oCont))
		{
            ...
            BX.bindDelegate(
                document.body, 'dblclick', {className: 'dnd-order-draggable-control' },
                function(e){
                    if(!e)
                        e = window.event;

                    var id = 'CATEGORY_'+ this.getAttribute('data-value');

                    var elems = document.getElementsByClassName("bxcompprop-item");
                    for (var i = 0; i < elems.length; i++) {
                        if(elems[i].getAttribute('data-bx-comp-group-id') == id){
                            elems[i].click();
						}
                    }
