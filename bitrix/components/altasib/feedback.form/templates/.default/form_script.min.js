var ALXpopup={timerId:"",timerReId:"",minw:500,param:"",removeWindow:true,fixed:false,canClearBlock:false,init:function(param)
...
return true;},autoResize:function()
{var oPopup=this;if($('.alx-feedb-data').length<1||$('.modal_form').css('position')!='fixed')
{clearInterval(oPopup.timerReId);return;}
if(($(window).height()>$('.modal_form').outerHeight()+100)&&this.param.fixed)
$('.modal_form').css({'top':(($(window).height()-$('.modal_form').outerHeight())/ 2-50)+'px','marginTop':'0','position':'fixed'});else
$('.modal_form').css({'position':'absolute','marginTop':$(window).scrollTop()+100+'px','top':'0'});},update:function()
{var oPopup=this;if($('.alx-feedb-data').length<1)
...
$('.modal_form').css({'opacity':1});},1000);oPopup.timerReId=setInterval(function(){oPopup.autoResize();},400);},close:function()
{var oPopup=this;var animClass=this.param.popupAnimation[1];$('.alx-feedb-data').css({'maxHeight':($(window).height()-100)+'px'});$('.modal_form').addClass(animClass);$('.alx_fb_overlay, .alx_fedb_wait, .modal_form').fadeOut(400);setTimeout(function(){if($('.modal_form').css('z-index')>2000)
...
{var oPopup=this;if($('.alx-feedb-data').length<1){$('.alx_cont_popup').html('<div class="alx-feedb-data alx-feedb-data-errorLoading">Loading error. Try again.<br /><a href="" onClick="ALXpopup.close(); return false" class="alx-feedb-data-close">close window</a></div>');oPopup.update();}},load_contentAjax:function()
{if(typeof this.param.data!='undefined'){var oPopup=this;$.ajax({type:'POST',data:this.param.data,url:this.param.url,async:true,success:function(data){$('.alx_cont_popup').html('<div class="alx-feedb-data">'+data+'</div>');$('.alx-feedb-data').find("form").submit(function(){oPopup.send_dataAjax(this);return false;});}});}},reload_contentAjax:function()
...
{var oPopup=this;var w=typeof param.width!=='undefined'?param.width:300;if((w+"").split('%').length==1){w=(w+"").split('px');var mLeft=0-parseInt(w[0])/2+'px';}else{w=(w+"").split('%');var mLeft=0-parseInt(w[0])/2+'%';}
...
$('.modal_form .alx-feedb-data,.alx_feed_back').empty();$('.modal_close_ok').click(function(){oPopup.close();});},send_dataAjax:function(form)
{var oPopup=this;var animClass=this.param.popupAnimation[2];var url=$(form).attr('action');url=typeof url!=='undefined'?url:"";var bTypeFD=false;if(typeof FormData=='function'){var FORM_DATA=new FormData(form);bTypeFD=true;}else
var FORM_DATA=$(form).serializeArray();$.ajax({url:url,type:'POST',data:FORM_DATA,cache:false,processData:(bTypeFD?false:true),contentType:(bTypeFD?false:'application/x-www-form-urlencoded;charset=UTF-8'),success:function(data){var sID=$(form).attr('name').split('_')[2];var sDelim='<!--REFRESH_SECTION-->';var pos=data.indexOf(sDelim);var errData=data,formData=false;if(pos>-1){errData=data.substr(0,pos);formData=data.substr(pos+sDelim.length);}
if(formData){$('#alx_feed_back_'+sID+'.alx_feed_back').html(formData);}
if(typeof sID!='undefined'&&$('#alx_feed_back_'+sID).siblings(".alx-feedb-error").length>0){$('#alx_feed_back_'+sID).siblings(".alx-feedb-error").html(errData).addClass(animClass);}
else if(typeof sID!='undefined'&&$('#alx_feed_back_'+sID).parent().parent().siblings(".alx-feedb-error").length>0){$('#alx_feed_back_'+sID).parent().parent().siblings(".alx-feedb-error").html(errData).addClass(animClass);}
else{$('.alx-feedb-error').html(errData).addClass(animClass);}
