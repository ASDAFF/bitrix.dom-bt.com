...
if(isset($arParams["REPLACE_PALETTE"]) && is_array($arParams["REPLACE_PALETTE"])) {
    $defaultPalette = $arParams["REPLACE_PALETTE"];
}

if(isset($arParams["UNSHIFT_PALETTE"]) && is_array($arParams["UNSHIFT_PALETTE"])) {
    array_unshift($defaultPalette, $arParams["UNSHIFT_PALETTE"]);
}

if(isset($arParams["PUSH_PALETTE"]) && is_array($arParams["PUSH_PALETTE"])) {
    array_push($defaultPalette, $arParams["UNSHIFT_PALETTE"]);
}

$arResult["PALETTE"] = $defaultPalette;
$arResult["PALETTE_JSON"] = Json::encode($defaultPalette);
