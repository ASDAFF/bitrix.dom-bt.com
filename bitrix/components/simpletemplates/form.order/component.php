...
$arParams["FORM_SUFFIX"] = trim($arParams["FORM_SUFFIX"]);
if($arParams["FORM_SUFFIX"] == '') {
	$arParams["FORM_SUFFIX"] = 'smt_order_form';
}
...
{
	// process POST data
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit_" . $arParams["FORM_SUFFIX"]]) || !empty($_REQUEST["iblock_apply_" . $arParams["FORM_SUFFIX"]])))
	{
		...
		if (empty($arResult["ERRORS"]))
		{
			if ($arParams["ELEMENT_ASSOC"] == "PROPERTY_ID"){
				...
				$arUpdatePropertyValues[$arParams["ELEMENT_ASSOC_PROPERTY"]] = $arParams['OBJECT_ID'];
			}
			...
			else
			{
				...
				if (!$arParams["ID"] != $oElement->Add($arUpdateValues, $bWorkflowIncluded, true, $arParams["RESIZE_IMAGES"]))
				{
					...
					$arPropFields = array();
					$arPropText = array();
					foreach($arResult["PROPERTY_LIST_FULL"] as $code=>$value) {
						if(intval($code) > 0) {
							if ($arUpdateValues["PROPERTY_VALUES"][$code]) {
								$arPropFields['IBLOCK_' . $value["CODE"]] = $arUpdateValues["PROPERTY_VALUES"][$code];
								$arPropText[] = $value["NAME"] . ": " . $arUpdateValues["PROPERTY_VALUES"][$code];
							}

							if ($value["CODE"] == "OBJECT_ID") {
								if($arUpdateValues["PROPERTY_VALUES"][$code] > 0) {
									$res = CIBlockElement::GetByID(intval($arUpdateValues["PROPERTY_VALUES"][$code]));
									if($arItemResult = $res->GetNext()) {
										$arPropFields['IBLOCK_' . $value["CODE"] . '_NAME'] = $arItemResult["NAME"];
										$arPropText[] = $arItemResult["NAME"];
									}
								}
							}
						}
					...
					$arFields = Array(
						"AUTHOR" => $arUpdateValues["NAME"],
						"AUTHOR_EMAIL" => $arUpdateValues["PROPERTY_VALUES"]['179'],
						"EMAIL_TO" => $arParams["EMAIL_TO"],
						"IBLOCK_DETAIL_TEXT" => $arUpdateValues["DETAIL_TEXT"],
						"TEXT" => $arUpdateValues["DETAIL_TEXT"] . "\n" . implode("\n\n", $arPropText),
					);
					...
					if(!empty($arParams["EVENT_MESSAGE_ID"]))
					{
						foreach($arParams["EVENT_MESSAGE_ID"] as $v) {
							if(IntVal($v) > 0) {
								CEvent::Send($arParams["EVENT_NAME"], SITE_ID, array_merge($arFields, $arPropFields), "N", IntVal($v));
							}
						}
					}
					else {
						CEvent::Send($arParams["EVENT_NAME"], SITE_ID, array_merge($arFields, $arPropFields));
					}
				}

				if (!empty($_REQUEST["iblock_apply_" . $arParams["FORM_SUFFIX"]]) && strlen($SEF_URL) > 0)
				{
		...
		if (empty($arResult["ERRORS"]))
		{
			if (!empty($_REQUEST["iblock_submit_" . $arParams["FORM_SUFFIX"]]))
			{
			...
			$sRedirectUrl .= ($sAction == "ADD")?"ADD_SUCCESS":"EDIT_SUCCESS";

			foreach($arResult["PROPERTY_LIST_FULL"] as $code=>$value) {
				if(intval($code) > 0) {
					if ($value["CODE"] == "OBJECT_ID") {
						if($arUpdateValues["PROPERTY_VALUES"][$code] > 0) {
							$sRedirectUrl .= "&OBJECT_ID=" . intval($arUpdateValues["PROPERTY_VALUES"][$code]);
						}
					}
				}
			}
