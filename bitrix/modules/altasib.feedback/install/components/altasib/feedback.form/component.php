...
if(!empty($_POST["TARGET_ELEMENT_ID"]))
{
        ...
        $res = CIBlockElement::GetList([], ["ID" => "$target", "CHECK_PERMISSIONS" => "Y"], false, false, ["IBLOCK_ID", "ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_PICTURE", "DETAIL_PICTURE"]);

        if ($ob = $res->Fetch())
        {
                if ($arResult["RIGHT_TO_EDIT"] = CIBlockElementRights::UserHasRightTo($ib["ID"], $ob["ID"], "element_edit"))
                        $arResult["ELEMENT_EDIT_LINK"] = $SERVER["DOCUMENT_ROOT"]."/bitrix/admin/iblock_element_edit.php?IBLOCK_ID=".$ib["ID"]."&type=".$ib["IBLOCK_TYPE_ID"]."&ID=".$ob["ID"]."&lang=".LANGUAGE_ID."&find_section_section=".$ob["IBLOCK_SECTION_ID"]."&WF=Y";
                else
                ...
                if(empty($ob["PREVIEW_PICTURE"]))
                {
                        ...
                        {
                                $arResult["POPUP_PICTURE"] = CFile::ResizeImageGet($ob["DETAIL_PICTURE"], Array("width" => 290, "height" => 180), BX_RESIZE_IMAGE_PROPORTIONAL);
                        }
                }
                else
                {
                        $arResult["POPUP_PICTURE"] = CFile::ResizeImageGet($ob["PREVIEW_PICTURE"], Array("width" => 290, "height" => 180), BX_RESIZE_IMAGE_PROPORTIONAL);
                }
        }
        else
        {
