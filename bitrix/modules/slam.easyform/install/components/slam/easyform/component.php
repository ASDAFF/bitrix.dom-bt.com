...
$http = (CMain::IsHTTPS()) ? "https://" . SITE_SERVER_NAME : "http://" . SITE_SERVER_NAME;
...
if ($request->isPost() && $arParams['FORM_ID'] == $request['FORM_ID']) {
        ...
        if (is_array($field)) {
            ...
            if ($fieldInfo['TYPE'] == 'file') {
                ...
                foreach ($fieldFiles as $strVal) {
                    if($strVal){
                        $arPostFields[$key][] = $http . CFile::GetPath($strVal);
                    }
                }
            } else {
    ...
    if (empty($arMessageError) && $arParams['ENABLE_SEND_MAIL']) {
        ...
        if ($arParams['REPLACE_FIELD_FROM'] && $arPostFields['EMAIL'] && strlen($arPostFields['EMAIL']) > 0) {
                    ...
                    if ($fieldCode != 'NO_WRITE') {
					    ...
					    if($arFormField = $arResult['FORM_FIELDS'][$key]['TYPE'] == 'file' && $val){
                            ...
                            foreach($files as $file){
                                $arFiles[] = CFile::MakeFileArray(str_replace($http, $_SERVER['DOCUMENT_ROOT'], $file));
                            }
    ...
    if (!empty($arMessageError)) {
        ...
        $result = array('result' => 'ok', 'message' => $arParams['OK_TEXT'], 'additiona_msg' => $arAdditionalMessage);
    }
