...
$arComponentParameters = array(
	...
	"PARAMETERS" => array(
		...
		"EMAIL_TO" => Array(
			"NAME" => GetMessage("SIMPLET_EMAIL_TO"),
			"TYPE" => "STRING",
		...
		"EVENT_MESSAGE_ID" => Array(
			"NAME" => GetMessage("SIMPLET_EMAIL_TEMPLATES"),
			"TYPE"=>"LIST",
			"VALUES" => $arEvent,
			"DEFAULT"=>"",
			"MULTIPLE"=>"Y",
		...
		"CUSTOM_FORM_SUBMIT" => array(
			"PARENT" => "PARAMS",
			"TYPE" => "TEXT",
			"NAME" => GetMessage("SIMPLET_CUSTOM_FORM_SUBMIT"),
		),
		"REDIRECT_URL" => array(
			"PARENT" => "PARAMS",
			"TYPE" => "TEXT",
			"NAME" => GetMessage("SIMPLET_REDIRECT_URL"),
		),
		"AJAX_URL" => array(
			"PARENT" => "PARAMS",
			"TYPE" => "TEXT",
			"NAME" => GetMessage("SIMPLET_AJAX_URL"),
		),
		"FORM_SUFFIX" => array(
			"PARENT" => "PARAMS",
			"TYPE" => "TEXT",
			"NAME" => GetMessage("SMT_COMP_ORDER_FORM_SUFFIX"),
		),
		"SHOW_FORM_BORDER" => array(
			"PARENT" => "VISUAL",
			"TYPE" => "CHECKBOX",
			"NAME" => GetMessage("SMT_COMP_ORDER_SHOW_FORM_BORDER"),
		),
	),
);
...
foreach ($arVirtualProperties as $key => $title) {
	if (!empty($arCurrentValues['PROPERTY_CODES']) && in_array($key, $arCurrentValues['PROPERTY_CODES'])) {
		$fieldsOrder[] = $key;
		$fieldsJsData[$key] = $title;
	}
}

$arComponentParameters["PARAMETERS"]["FIELDS_ORDER"] = array(
	'PARENT' => 'VISUAL',
	'NAME' => GetMessage("SMT_CP_FORM_ORDER_FIELDS_ORDER"),
	'TYPE' => 'CUSTOM',
	'JS_FILE' => $componentPath.'/settings/dragdrop_order/script.js',
	'JS_EVENT' => 'initDraggableOrderControl',
	'JS_DATA' => Json::encode($fieldsJsData),
	'DEFAULT' => implode(',', $fieldsOrder),
);
