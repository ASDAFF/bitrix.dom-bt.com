...
$yandexPlacemarks = array();
foreach ($arResult["ITEMS"] as $count=>$arItem) {
    if (strlen($arItem["PROPERTIES"]["MAP_YANDEX"]["VALUE"]) > 0 && $arItem["PROPERTIES"]["MAP_SCALE"]['VALUE']) {
        $arCoords = explode(',', $arItem["PROPERTIES"]["MAP_YANDEX"]['VALUE'], 2);
        $lat = doubleval($arCoords[0]);
        $lng = doubleval($arCoords[1]);
        $placemark = array(
            'SCALE' => $arItem["PROPERTIES"]["MAP_SCALE"]['VALUE'],
            'LON' => $lng,
            'LAT' => $lat,
            'TEXT' => $arItem["PREVIEW_TEXT"]
        );
        $yandexPlacemarks[] = $placemark;
        $arResult["ITEMS"][$count]["MAP_VALUE"] = $placemark;
    }
}

$arResult["MAP_DATA"] = "";
if(!empty($yandexPlacemarks) && $yandexPlacemarks[0]) {
    $arResult["MAP_DATA"] = serialize(array(
        'yandex_lat' => $yandexPlacemarks[0]["LAT"],
        'yandex_lon' => $yandexPlacemarks[0]["LON"],
        'yandex_scale' => $yandexPlacemarks[0]["SCALE"],
        'PLACEMARKS' => $yandexPlacemarks
    ));
}