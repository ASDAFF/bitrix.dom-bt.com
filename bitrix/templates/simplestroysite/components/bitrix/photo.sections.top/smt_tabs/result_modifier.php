...
if($arParams["PICTURE_RESIZE"] == "Y") {
    foreach($arResult["SECTIONS"] as $sectionCount=>$arSection) {
        foreach ($arSection["ITEMS"] as $count=>$arItem) {
            if ($arItem[$arParams["PICTURE_RESIZE_SOURCE"]]) {
                ...
                $arResult["SECTIONS"][$sectionCount]["ITEMS"][$count]["PREVIEW_PICTURE"]["SRC"] = $file["src"];
                $arResult["SECTIONS"][$sectionCount]["ITEMS"][$count]["PREVIEW_PICTURE"]["WIDTH"] = $file["width"];
                $arResult["SECTIONS"][$sectionCount]["ITEMS"][$count]["PREVIEW_PICTURE"]["HEIGHT"] = $file["height"];
            }
        }
    }
}
...
foreach ($arResult["SECTIONS"] as $count=>$arItem) {
    $arItem['PICTURE'] = intval($arItem['PICTURE']);
    $arResult["SECTIONS"][$count]['PICTURE'] = (0 < $arItem['PICTURE'] ? CFile::GetFileArray($arItem['PICTURE']) : false);
    $arItem['DETAIL_PICTURE'] = intval($arItem['DETAIL_PICTURE']);
    $arResult["SECTIONS"][$count]['DETAIL_PICTURE'] = (0 < $arItem['DETAIL_PICTURE'] ? CFile::GetFileArray($arItem['DETAIL_PICTURE']) : false);
}

if($arParams["SECTION_PICTURE_RESIZE"] == "Y") {
    foreach ($arResult["SECTIONS"] as $count=>$arItem) {
        if ($arItem[$arParams["SECTION_PICTURE_RESIZE_SOURCE"]]) {
            $file = CFile::ResizeImageGet($arItem[$arParams["SECTION_PICTURE_RESIZE_SOURCE"]], array('width'=> (int) $arParams["SECTION_PICTURE_RESIZE_WIDTH"], 'height'=> (int) $arParams["SECTION_PICTURE_RESIZE_HEIGHT"]), (int) $arParams["SECTION_PICTURE_RESIZE_MODE"], true);
            $arResult["SECTIONS"][$count]["PREVIEW_PICTURE"]["SRC"] = $file["src"];
            $arResult["SECTIONS"][$count]["PREVIEW_PICTURE"]["WIDTH"] = $file["width"];
            $arResult["SECTIONS"][$count]["PREVIEW_PICTURE"]["HEIGHT"] = $file["height"];
        }
    }
}
...
foreach($arResult["SECTIONS"] as $sectionCount=>$arSection) {
    foreach ($arSection["ITEMS"] as $count=>$arItem) {
        ...
        foreach($arParams["PROPERTY_CODE"] as $pid)
        {
            ...
            {
                $arResult["SECTIONS"][$sectionCount]["ITEMS"][$count]["DISPLAY_PROPERTIES"][$pid] = CIBlockFormatProperties::GetDisplayValue($arResult, $prop, "news_out");
            }
        }
    }
}
