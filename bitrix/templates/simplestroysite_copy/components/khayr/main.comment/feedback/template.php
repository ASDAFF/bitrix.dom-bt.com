		...
		<div class="userText">
			<div style="display:  inline;">
				...
				<div class="smt-response-item__text">
					<span class="smt-response-item__quote smt-response-item__quote_left">«</span>
					<div class="smt-read-more-text">
						<p><?=$arItem["PUBLISH_TEXT"]?></p>
					</div>
					<span class="smt-response-item__quote smt-response-item__quote_right">»</span>
				</div>
				<?
            ...
            <div class="userInfo">
				<span><?=$arItem["AUTHOR"]["FULL_NAME"]?></span>
	...
	<div class="form comment main_form"<?=($arResult["POST"]["PARENT"] > 0 && !$arResult["SUCCESS"] ? " style='display: none;' " : "")?>>
		<?if ($arResult["CAN_COMMENT"]) {?>
			<form enctype="multipart/form-data" action="<?=$GLOBALS["APPLICATION"]->GetCurUri()?>" method='POST' onsubmit='return KHAYR_MAIN_COMMENT_validate(this);'>
            	<div class="row">
                	<div class="col-md-6">
                    <label class="text-uppercase" for="NONUSER">ВАШЕ ИМЯ *</label>
					<input type="text" name='NONUSER' value='<?=$arResult["POST"]["NONUSER"]?>' <?=($arResult["USER"]["ID"] ? "readonly='readonly'" : "")?> placeholder="<?=GetMessage("KHAYR_MAIN_COMMENT_FNAME")?>" class="form-control" required />
                    </div>
                    <div class="col-md-6">
                    <?if ($arResult["LOAD_EMAIL"]) {?>

					<label class="text-uppercase" for="EMAIL">EMAIL *</label>
					<input type="text" name='EMAIL' <?=($arResult["USER"]["ID"] ? "value='".$arResult["USER"]["EMAIL"]."' readonly='readonly'" : "value='".$arResult["POST"]["EMAIL"]."'")?> placeholder="<?=GetMessage("KHAYR_MAIN_COMMENT_EMAIL")?>" class="form-control" required />
				<?}?>
                    </div>
				...
				<?if ($arResult["LOAD_AVATAR"]) {?>
					<input type="file" name='AVATAR' value='' placeholder="<?=GetMessage("KHAYR_MAIN_COMMENT_AVATAR")?>" class="form-control" />
				<?}?>
				
				<?foreach ($arParams["ADDITIONAL"] as $additional) {?>
					<input type="text" name='<?=urlencode($additional)?>' value='<?=$arResult["POST"][$additional]?>' placeholder="<?=$additional?>" class="form-control" />
				<?}?>
				<div class="clear"></div>
				<?if ($arParams["LOAD_MARK"]) {?>
					<?=GetMessage("KHAYR_MAIN_COMMENT_MARK")?>:
				...
				<?}?>
					...
					<label class="text-uppercase" for="MESSAGE">ТЕКСТ ОТЗЫВА *</label>

				<textarea name="MESSAGE" rows="5" placeholder='<?=GetMessage("KHAYR_MAIN_COMMENT_MESSAGE")?>' class="form-control"><?=$arResult["POST"]["MESSAGE"]?></textarea>
				...
				<?if ($arParams["LEGAL"]) {?>
					...
					<label for="LEGAL_main_form" class="smt-form-agreement"><?=$arParams["LEGAL_TEXT"]?></label>
					<div class="clear pt10"></div>
				<?}?>
				<input type="submit" class="btn btn-block smt-btn smt-btn_shadow text-uppercase" value="<?=GetMessage("KHAYR_MAIN_COMMENT_ADD")?>" />
			</form>
		<?} else {?>
