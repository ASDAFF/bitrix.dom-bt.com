...
<?/*$APPLICATION->IncludeComponent("underware:catalog.filter", "template", Array(
	"COMPONENT_TEMPLATE" => ".default",
		"IBLOCK_TYPE" => "smt_company",	// Тип инфоблока
		...
		"SIZE_SORT" => "2",
		"NUMBER_WIDTH" => "20",	// количество в свернутом виде
		...
		"BEDROOMS_SORT" => "6",
		"PROPERTY_POP_CATS_VALUE" => array(	// значения свойства "Популярные категории"
			0 => "80",
			1 => "81",
			2 => "82",
			3 => "83",
		...
		"POP_CATS_SORT" => "1",	// Сортировка"Популярные категории"
		"RG_FILTER" => $rg_filter
	),
	false
);*/?>
        ...
        <div class="bx-filter-section">
            <form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
                ...
                <div class="row">
                    <?foreach($arResult["ITEMS"] as $key=>$arItem) {
                        if($arItem["ID"] == "152") { ?>
                            <div class="rg_filter_name"><?=$arItem["NAME"]?></div>
                            <div class="col-md-12 bx-filter-parameters-box bx-active">
...
<div class="bx-filter <?=$templateData["TEMPLATE_CLASS"]?> <?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL") echo "bx-filter-horizontal"?>">
<div class="bx-filter-section">
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter" style="background: #efefef; padding: 0 15px 15px;">
			<?foreach($arResult["HIDDEN"] as $arItem):?>
			<input type="hidden" name="<?echo $arItem["CONTROL_NAME"]?>" id="<?echo $arItem["CONTROL_ID"]?>" value="<?echo $arItem["HTML_VALUE"]?>" />
			<?endforeach;?>
			<div class="row">
                ...
                foreach($arResult["ITEMS"] as $key=>$arItem)
                {
                    if($arItem["ID"] == "152") { ?>
                        <?$rg_unset = $key?>
                        <div id="rg_top_filter_wrapp" class="col-xs-12 col-md-12">
                            <?if(
                                empty($arItem["VALUES"])
                                || isset($arItem["PRICE"])
                            )
                            ...
                            <?php if (($arItem["DISPLAY_TYPE"] == "A") || ($arItem["DISPLAY_TYPE"] == "CHECKBOX")) { ?>
                                <div class="col-xs-12 col-md-12 bx-filter-parameters-box bx-active">
                            <?} else { ?>
                                <? if  ( $ii == 1 ) {?> <div class="checkcolumn  col-md-12 bx-filter-parameters-box bx-active"> <? $ll="12"; }
                                    if  ( $ii == 2 ) { $ll="3"; }
                                    if  ( $ii == 3 ) { $ll="3"; }
                                    if  ( $ii == 4 ) { $ll="3"; }
                                    ?>
                ...
                <?unset($arResult["ITEMS"][$rg_unset]);?>
                <?$ii = 0;?>
                <div class="row" style="margin-left: 0; margin-right: 0;display: inline-block;width: 100%;margin-top: 15px;">
                    <div class="<?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL"):?>col-sm-6 col-md-4<?else:?>col-lg-12<?endif?> bx-filter-title">
                        <span class="smt-header-underline-center" style="color:#fff; background-color: #1fa0da;padding: 14px;margin-left: -15px;margin-bottom: 0;display: inline-block;margin-top: -15px;">
				...
				<?foreach($arResult["ITEMS"] as $key=>$arItem)
				{
					...
					<?} else { ?>
					  ...
					  <!-- блок с параметрами -->
                        <? if($arItem["ID"] == "152") { ?>
                          <div class="col-md-<? echo $ll; ?> bx-filter-parameters-box bx-active rg_filter_wrapp">
                        <? } else { ?>
                              <div class="col-md-<? echo $ll; ?> bx-filter-parameters-box bx-active">
                        <? } ?>
					<? } ?>
...
<script>
    $(document).ready(function () {
        $('#modef a').on('click', function () {
            var filter_height = $('#rg_top_filter_wrapp').height() - 85;
...
<script>
    $('#sl').on('click', function (e) {
        ...
        if (!$(this).hasClass('closed')) {
            ...
            $('#rg_top_filter_wrapp .checkbox').each(function (i, elem) {
                if (i >= 20) {
    ...
    /*$('#rg_top_filter_wrapp .bx-filter-parameters-box-container .checkbox input[type=checkbox]').on('click', function (e) {
        $('#rg_top_filter_wrapp .checkbox.hidden').removeClass('hidden');
        if (!$(this).hasClass('closed')) {
            $(this).html('Показать все').addClass('closed');
            $('#rg_top_filter_wrapp .checkbox').each(function (i, elem) {
