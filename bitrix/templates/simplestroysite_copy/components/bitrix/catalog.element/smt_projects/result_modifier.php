...
if (!empty($arResult['PROPERTIES'][$galleryCode]['VALUE']) && is_array($arResult['PROPERTIES'][$galleryCode]['VALUE']))
{
    ...
    if($arParams["PICTURE_RESIZE_GALLERY"] == "Y") {
        ...
        //добавлен код для ресайза больших фото в галерее:
        foreach ($arResult[$galleryCode] as $count=>$arItem) {
            if ($arResult['DETAIL_PICTURE']) {
                $file = CFile::ResizeImageGet($arItem['DETAIL_PICTURE'], array('width'=> 1600, 'height'=> 1200), $arParams["PICTURE_RESIZE_MODE"], true);
                $arResult[$galleryCode][$count]['DETAIL_PICTURE']['SRC'] = $file['src'];
                $arResult[$galleryCode][$count]['DETAIL_PICTURE']['WIDTH'] = $file['width'];
                $arResult[$galleryCode][$count]['DETAIL_PICTURE']['HEIGHT'] = $file['height'];
            }
        }
    } else {
        foreach ($arResult[$galleryCode] as $count=>$arItem) {
...
$arFilter = array(
	//"ACTIVE" => "Y",
	"IBLOCK_ID" => '36',
	"PROPERTY_OBJECT_ID" => $arResult["ID"]
);
...
$arNavigation = CDBResult::GetNavParams($arNavParams);
	...
	$rsElements = CIBlockElement::GetList(
		...
		false,
		$arNavParams,
		array("ID", "IBLOCK_ID","NAME", "DATE_ACTIVE_FROM", "PREVIEW_TEXT", "DETAIL_TEXT", "DATE_CREATE", "CREATED_BY")
	);
	while($obElement = $rsElements->GetNextElement()) {
		...
		$arReview["ITEMS"][] = $arElement;
	}

	//NAVIGATION//
	$arReview["NAV_STRING"] = $rsElements->GetPageNavStringEx($navComponentObject, "", "reviews");
