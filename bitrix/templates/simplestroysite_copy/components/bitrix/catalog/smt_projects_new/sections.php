<? if ( ! defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true){
	...
	if (isset($_GET['prop']) && isset($_GET['value'])){
        global $arrFilter;
		$arrFilter = array(
			$_GET['prop']=>array($_GET['value'])
        );
		if (isset($_REQUEST['AJAX_CALL']) && $_REQUEST['AJAX_CALL'] == 'Y'){
			unset($arrFilter[ $_GET['prop'] ]);
			unset($_GET['prop']);
			unset($_GET['value']);
		}
	}
	...
	<? if ($isFilter): ?>
		<? if ($APPLICATION->GetCurDir() == "/projects/"){ ?>
            ...
            <div class="smt-widget smt-widget_margin">
                <a class="btn smt-btn btn-block hidden-sm hidden-md hidden-lg" role="button" data-toggle="collapse"
                   href="#bx_filter_catalog" aria-expanded="false" aria-controls="bx_filter_catalog">
...
<? if ($isSidebar): ?>
<!-- projects grid -->
<div id="rg_filter_scroll" class="col-xs-12 col-sm-12 col-md-12 col-lg-12<? if ($isSidebarRight): ?> col-md-pull-3 col-lg-pull-3<? endif ?>">
	<? else: ?>
    <div id="rg_filter_scroll" class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<? endif ?>
        ...
        $currentSortField = $parameters["ELEMENT_SORT_FIELD"];
        ...
        $inverseSortOrder = ($currentSortOrder === 'asc') ? 'desc' : 'asc';
			...
			$parameters["ELEMENT_SORT_FIELD"] = $currentSortField;
			$parameters["ELEMENT_SORT_ORDER"] = $currentSortOrder;?>
