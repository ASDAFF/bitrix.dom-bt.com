	...
	$.fn.sprintVideoGallery = function(options) {
		...
		SprintVideoGallery.prototype = {
			...
			setLargeThumbnail: function($thumbnail, onPreloader) {
				...
				if ( !this.largeThumbnailsCache[src] ) {
					...
					if (type === 'youtube') {
						$largeThumbnail = $('<iframe src="https://www.youtube.com/embed/' + src + '?autoplay=1" frameborder="0" allowfullscreen></iframe>');
					}

				} else {
					$largeThumbnail = this.largeThumbnailsCache[src];
				}

				if ( this.$largeThumbnail ) {
					this.youtubeAutoplayOff(this.$largeThumbnail);
					this.$largeThumbnail.hide();
				}

				this.$largeThumbnail = $largeThumbnail;

				if ( Object.keys( this.largeThumbnailsCache ).length ) {
					this.youtubeAutoplayOn(this.$largeThumbnail);
				} else {
					this.youtubeAutoplayOff(this.$largeThumbnail);
				}

				this.$largeThumbnail.show();

				if ( !this.largeThumbnailsCache[src] ) {
					this.largeThumbnailsCache[src] = $largeThumbnail;
					this.$largeThumbnailWrapper.append($largeThumbnail);

					if ( onPreloader === 'undefined' || onPreloader !== false ) {
						if (this.$largeThumbnail.find('img')[0]) {
							this.$largeThumbnail.find('img')[0].onload = function() {
								this.hidePreloader();
							}.bind(this);
						} else {
							this.$largeThumbnail[0].onload = function() {
								this.hidePreloader();
							}.bind(this);
						}
			...
			youtubeAutoplayOn: function($iframe) {
				var iframeSrc,
					on = 'autoplay=1',
					off = 'autoplay=0';
				...
				if ( $iframe[0].tagName === 'IFRAME' ) {
					...
					$iframe.attr( 'src', iframeSrc.replace(off, on) );
				}
			},

			youtubeAutoplayOff: function($iframe) {
				var iframeSrc,
					on = 'autoplay=1',
					off = 'autoplay=0';
				...
				if ( $iframe[0].tagName === 'IFRAME' ) {
					...
					$iframe.attr( 'src', iframeSrc.replace(on, off) );
				}
			},

			youtubeToggleAutoplay: function($iframe) {
				var iframeSrc,
					on = 'autoplay=1',
					off = 'autoplay=0';
			...
			thumbnailsClick: function(e) {
				var $target = $(e.target),
					...
					self = this,
					$largeThumbnailClone,
					$largeThumbnailCahceElem;

				if ($selected.length ) {
					$largeThumbnailCahceElem = self.largeThumbnailsCache[ $selected.find('> div').attr('data-src') ];
					if ( $largeThumbnailCahceElem && $largeThumbnailCahceElem[0] === self.$largeThumbnail[0] ) return;
