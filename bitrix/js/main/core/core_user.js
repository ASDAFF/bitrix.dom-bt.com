...
{
	lastseen = BX.type.isDate(lastseen) ? new Date(lastseen.getTime()) : BX.type.isNumber(lastseen) ? new Date(lastseen * 1000) : new Date(0);
	now = BX.type.isDate(now) ? new Date(now.getTime()) : BX.type.isNumber(now) ? new Date(now * 1000) : new Date();
	utc = !!utc;
